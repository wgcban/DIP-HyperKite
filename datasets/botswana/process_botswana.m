...
for l = 1:floor(L/patch_size)
    for w = 1:floor(W/patch_size)
        ...
        folder_name = strcat('botswana_', num2str(count, '%02d'));
        file_name= fullfile('./botswana', folder_name);
        mkdir(file_name)
        ...
        % Saving ref, y, pan
        save(strcat(file_name, '/botswana_', num2str(count, '%02d'),'.mat'), 'ref', 'y', 'pan');
        count=count+1;
    end
end
